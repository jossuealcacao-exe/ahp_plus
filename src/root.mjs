...
export function statePaths(resolved) {
  return {
    ...
    relay: path.join(resolved.stateRoot, 'relay'),
    relayOutbox: path.join(resolved.stateRoot, 'relay/outbox'),
    relayInbox: path.join(resolved.stateRoot, 'relay/inbox'),
    relayReceipts: path.join(resolved.stateRoot, 'relay/receipts'),
    identities: path.join(resolved.stateRoot, 'identities'),
    identitiesDevices: path.join(resolved.stateRoot, 'identities/devices'),
    secure: path.join(resolved.stateRoot, 'secure'),
    secureOutbox: path.join(resolved.stateRoot, 'secure/outbox'),
    secureInbox: path.join(resolved.stateRoot, 'secure/inbox'),
    secureReceipts: path.join(resolved.stateRoot, 'secure/receipts'),
